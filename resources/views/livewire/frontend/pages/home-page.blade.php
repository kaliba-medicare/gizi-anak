        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header border-0 align-items-center d-flex py-0 bg-secondary text-white">
                    ...
                    <div class="my-2">
                        <select id="year-selector" class="form-select form-select-sm" onchange="updateYear()">
                            @foreach ($listYears as $year)
                                <option value="{{ $year }}"
                                    {{ $year == $filterYear ? 'selected' : '' }}>{{ $year }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script>
        ...
        function updateYear() {
            ...
            Livewire.emit('updateYear', selectedYear); // Use Livewire directly
        }
        ...
        // Ambil semua bulan unik dari semua data
        ...
        // Bar chart: Per bulan, tiap series adalah type gizi
        ...
        // Line chart: sama seperti bar, tapi dengan type "line"
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            async function loadGeoJson(boundaryType) {
                ...
                try {
                    ...
                    // Fetch the stunting data for the selected boundary
                        ...
                        data.features.forEach(feature => {
                            ...
                            L.marker([coords[1], coords[0]], {
                                icon: L.divIcon({
                                    ...
                                    html: `<div style="background: rgba(255, 255, 255, 0.8); padding: 5px; border-radius: 5px; font-weight: bold;">${count}</div>`,
                                    iconSize: [50, 20]
                                })
                            }).addTo(stuntingMap);
                        });
