<div class="container-fluid">
    ...
    {{-- <ul>
        <strong>{{ count($dataGizi_posyandu) }}</strong> Posyandu Data Gizi Anak:
        @foreach($dataGizi_posyandu as $item)
            <li>{{ $item->posyandu }}</li>
        @endforeach
    </ul>

    @php
        $giziNames = $dataGizi_posyandu->pluck('posyandu')->toArray();
        $posyanduNames = $dataPosyandu->pluck('nama_posyandu')->toArray();

        $sameNames = array_intersect($giziNames, $posyanduNames);
        $onlyInGizi = array_diff($giziNames, $posyanduNames);
        $onlyInDb = array_diff($posyanduNames, $giziNames);
    @endphp

    <table border="1" cellpadding="10" cellspacing="0" style="width: 100%; border-collapse: collapse; text-align: left;">
        <thead style="background-color: #f0f0f0;">
            <tr>
                <th style="width: 33%; background-color: #e0ffe0;">✅ Data yang Sama ({{ count($sameNames) }})</th>
                <th style="width: 33%; background-color: #fffbe0;">🟠 Hanya di Data Gizi ({{ count($onlyInGizi) }})</th>
                <th style="width: 33%; background-color: #e0f0ff;">🔵 Hanya di Data Posyandu DB ({{ count($onlyInDb) }})</th>
            </tr>
        </thead>
        <tbody>
            @php
                $maxRows = max(count($sameNames), count($onlyInGizi), count($onlyInDb));
                $sameNames = array_values($sameNames);
                $onlyInGizi = array_values($onlyInGizi);
                $onlyInDb = array_values($onlyInDb);
            @endphp
...
@push('scripts')
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const stuntingMap = L.map('map').setView([-8.386027594276378, 116.23857670737101], 11);
            ...
            const MapControls = L.Control.extend({
                ...
                onAdd: function(map) {
                    ...
                    // Boundary selector
                    container.innerHTML = `
                        <label for="boundary-selector"><strong>Pilih Batas Wilayah</strong></label>
                        ...
                        <h5 class="card-title mb-3" style="margin-top: 10px;">Pilih Wilayah</h5>
                        ...
                        <button class="btn btn-primary btn-sm" id="reset-map" style="margin-top: 10px;">Reset Peta</button>
                    `;
                ...
                const marker = L.marker([lat, lng], {
                    ...
                    .bindPopup(`
                       <table class="table table-bordered">
                            <tr>
                                <th colspan="2" class="text-center bg-secondary"><strong>${pos.nama_posyandu}</strong></th>
                            </tr>
                            <tr>
